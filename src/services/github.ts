...
export class GitHubService {
  ...
  async searchCode(repoName: string, query: string, language?: string): Promise<SearchResult[]> {
    ...
    return response.data.items.map((item: any) => ({
      ...
      highlights: item.text_matches?.[0]?.matches?.map((m: any) => m.fragment) ?? []
    }));
  }
  
  async getFileContent(repoName: string, filePath: string): Promise<FileContent> {
    await this.rateLimiter.waitForSlot();
  ...
  async getDirectoryStructure(repoName: string, dirPath: string = '/'): Promise<RepoStructureItem[]> {
    await this.rateLimiter.waitForSlot();
  ...
  async searchIssues(repoName: string, query: string): Promise<{
    title: string;
    body: string;
    number: number;
    state: string;
