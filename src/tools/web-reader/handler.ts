...
import { parseHtml, htmlToMarkdown, htmlToText } from '../../utils/htmlParser.js';
import { logger } from '../../utils/logger.js';
import type { WebReaderInput, WebReaderOutput, LinkSummary } from './types.js';

export async function webReaderHandler(input: WebReaderInput): Promise<WebReaderOutput> {
  ...
  const formatFn = format === 'markdown' ? htmlToMarkdown : format === 'text' ? htmlToText : () => response.content;
  ...
  if (input.withLinksSummary) {
    result.links = parsed.links.map(l => ({ text: l.text, url: l.href })) as LinkSummary[];
  }
