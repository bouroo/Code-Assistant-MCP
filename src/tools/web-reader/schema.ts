...
export const WebReaderInputSchema = z.object({
  url: z.string()
    .url()
    .describe('The URL of the webpage to fetch and read'),
  
  returnFormat: z.enum(['markdown', 'text', 'json'])
    .default('markdown')
    .optional()
    .describe('Output format for the content'),
  
  retainImages: z.boolean()
    .default(true)
    .optional()
    .describe('Whether to retain image references in output'),
  
  withLinksSummary: z.boolean()
    .default(false)
    .optional()
    .describe('Include summary of all links found'),
  
  withImagesSummary: z.boolean()
    .default(false)
    .optional()
