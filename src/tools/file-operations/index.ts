...
export function registerFileOperationsTool(server: McpServer): void {
  server.registerTool(
    "file_operations",
    {
      ...
      description:
        "Perform file system operations safely. Supports read, write, list, delete, exists, and stat operations with path validation against allowed directories.",
      inputSchema: FileOperationsInputSchema,
    },
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    async (args: any) => {
      const input = FileOperationsInputSchema.parse(
        args,
      ) as FileOperationsInput;
