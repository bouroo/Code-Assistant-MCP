...
function validatePath(targetPath: string): string {
  ...
  if (!isAllowed) {
    throw new ForbiddenError(
      `Path '${targetPath}' is not within allowed directories: ${allowedPaths.join(", ")}`,
    );
  }
...
export async function fileOperationsHandler(
  input: FileOperationsInput,
): Promise<FileOperationsOutput> {
  logger.info("File operation request", {
  ...
  switch (input.operation) {
    ...
    default:
      throw new Error(
        `Unknown operation: ${(input as FileOperationsInput).operation}`,
      );
  }
}
...
async function handleRead(
  ...
  try {
    const content = await readFile(
      validatedPath,
      encodingType === "utf-8" ? "utf-8" : undefined,
    );
    const size = Buffer.isBuffer(content)
      ? content.length
      : Buffer.byteLength(content as string, "utf-8");

    return {
      operation: "read",
      content:
        encodingType === "base64" && !Buffer.isBuffer(content)
...
async function handleWrite(
  ...
  const finalContent =
    encodingType === "base64" ? Buffer.from(content, "base64") : content;
  ...
  return {
    operation: "write",
    success: true,
    bytesWritten: Buffer.isBuffer(finalContent)
      ? finalContent.length
      : Buffer.byteLength(finalContent, "utf-8"),
  };
}
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          ...
          // eslint-disable-next-line no-unused-vars
          const _resolvedBase = resolve(process.cwd());
