...
export async function codeExecuteHandler(input: CodeExecuteInput): Promise<CodeExecuteOutput> {
  logger.info('Code execute request', { language: input.language, codeLength: input.code.length });
  ...
  try {
    ...
    const { command, args, filename } = getCommandConfig(input.language, input.code, tmpDir);
    
    // Write main code file
    await writeFile(join(tmpDir, filename), input.code);
    
    // Execute the code
    const result = await executeCommand(command, args, input.input, timeoutMs, tmpDir);
...
function getCommandConfig(language: string, code: string, tmpDir: string): {
  command: string;
  args: string[];
  filename: string;
} {
  switch (language) {
    ...
    case 'sql':
      ...
      throw new Error('SQL execution is not supported. This tool executes code in the host environment, not against databases.');
